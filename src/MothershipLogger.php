...
use Mothership\Truncation\Truncation;
use Mothership\Payload\EncodedPayload;

class MothershipLogger extends AbstractLogger
{
    /**
    ...
    /**
     * Creates a new instance of the MothershipLogger.
     *
    ...
    /**
     * Returns a new {@see MothershipLogger} instance with a combination of the configs from the current logger updated
     ...
     * @throws Exception
     */
    public function scope(array $config): MothershipLogger
    {
        return new MothershipLogger($this->extend($config));
    }
     ...
     * @param bool              $isUncaught True if the error or exception was captured by a Mothership handler. Thus, it
    ...
    public function report(
        ...
        if ($this->disabled()) {
            $this->verboseLogger()->notice('Mothership is disabled');
    ...
    /**
     * Sends a report to the Mothership service.
     *
     ...
     * @param string|Stringable $toLog       The message or error to be sent to Mothership.
    ...
    protected function getPayload(
        ...
        $data    = $this->config->getMothershipData($level, $toLog, $context);
     ...
     * @param Response $response The response from the Mothership service.
     *
     * @return void
     */
