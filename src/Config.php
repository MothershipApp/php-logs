...
if (!defined('LOGS_INCLUDED_ERRNO_BITMASK')) {
    define(
        'LOGS_INCLUDED_ERRNO_BITMASK',
...
class Config
{
    ...
    private $accessToken;
    /**
     * @var string $enabled Enable / disable Logs SDK.
     */
     ...
     * sent through LogsLogger::log().
     */
    private $sendMessageTrace = false;
    
    /**
     * @var string (One of the \Psr\Log\LogLevel constants) How much debugging
     * info should be recorded in the Logs debug log file.
     * ($logsLogger->getDebugLogFile() => commonly /tmp/logs.debug.log.
    ...
    {
        $this->verbosity = Defaults::get()->verbosity();
    ...
    {
        if (isset($_ENV['LOGS_ACCESS_TOKEN']) && !isset($config['access_token'])) {
            $config['access_token'] = $_ENV['LOGS_ACCESS_TOKEN'];
        }
//        $this->utilities->validateString($config['access_token'], "config['access_token']", 32, false);
    ...
    private function setDataBuilder($config)
    {
        ...
        $exp = "Mothership\DataBuilderInterface";
        $def = "Mothership\DataBuilder";
        $this->setupWithOptions($config, "dataBuilder", $exp, $def, true);
    }

    private function setTransformer($config)
    {
        $expected = "Mothership\TransformerInterface";
    ...
    {
        $this->setupWithOptions($config, "filter", "Mothership\FilterInterface");
    }

    private function setSender($config)
    {
        $expected = "Mothership\Senders\SenderInterface";
        
        $default = "Mothership\Senders\CurlSender";
    ...
    {
        $exp = "Mothership\ScrubberInterface";
        $def = "Mothership\Scrubber";
    ...
    private function setAgentSenderOptions(&$config, $default)
    {
        ...
        $default = "Mothership\Senders\AgentSender";
    ...
    private function setFluentSenderOptions(&$config, $default)
    {
        ...
        $default = "Mothership\Senders\FluentSender";
    ...
    {
        $this->setupWithOptions($config, "responseHandler", "Mothership\ResponseHandlerInterface");
    }
    ...
    public function getLogsData($level, $toLog, $context)
    {
