...
use Mothership\Senders\AgentSender;
use Mothership\Senders\CurlSender;
use Mothership\Senders\SenderInterface;
use Throwable;
use Mothership\Senders\FluentSender;
...
class Config
{
     ...
     * try to be as close to the scenario where Mothership did not exist at
     * all in the code.
     * Default: true
     */
     ...
     * useful to someone trying to understand what Mothership is doing. The logged
     ...
     * an additional Mothership\Config::VERBOSE_NONE option which makes the SDK quiet
     ...
     * Default: Mothership\Config::VERBOSE_NONE
     */
     ...
     * Default: \Mothership\VerboseLogger
     */
    ...
    /**
     * @var \Mothership\Senders\SenderInterface
     */
     ...
     * sent through MothershipLogger::log().
     */
     ...
     * The class should implement {@see \Mothership\Truncation\StrategyInterface}.
     *
    ...
    /**
     * @var int The maximum number of items reported to Mothership within one
     * request.
     */
    ...
    private function setLogPayloadLogger(array $config): void
    {
        $this->logPayloadLogger = $config['log_payload_logger'] ??
            new Logger('mothership.payload', array(new ErrorLogHandler()));
    ...
    private function setVerboseLogger(array $config): void
    {
        if (isset($config['verbose_logger'])) {
            ...
            $this->verboseLogger = new Logger('mothership.verbose', array($handler));
        }
    ...
    {
        $this->minimumLevel = \Mothership\Defaults::get()->minimumLevel();
    ...
    private function setReportSuppressed(array $config): void
    {
        ...
        if (!$this->reportSuppressed) {
            $this->reportSuppressed = \Mothership\Defaults::get()->reportSuppressed();
        }
    }
    ...
    private function setRaiseOnError(array $config): void
    {
        if (array_key_exists('raise_on_error', $config)) {
            ...
            $this->raiseOnError = \Mothership\Defaults::get()->raiseOnError();
        }
    }
     ...
     *                          must implement {@see \Mothership\Truncation\StrategyInterface}. If null is given any custom
    ...
    public function getMothershipData($level, $toLog, $context)
    {
     ...
     *
     * @param \Mothership\ErrorWrapper $toLog
     *
     * @return bool
     */
    ...
    public function shouldSuppress(): bool
    {
        ...
        if (version_compare(PHP_VERSION, '8.0', 'ge') && $errorReporting === (E_ERROR | E_CORE_ERROR | E_COMPILE_ERROR | E_USER_ERROR | E_RECOVERABLE_ERROR | E_PARSE
        )) {
            return true;
        }
